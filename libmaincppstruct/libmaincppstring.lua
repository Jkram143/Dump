...
local StringApi = {
    ...
    EditString = function(self, newStr)
        ...
        elseif _stringLength > #bytes then
            ...
            gg.setValues(_bytes)
        elseif _stringLength < #bytes then
            self.address = libmain.cpp.MemoryManager.MAlloc(self.Fields._firstChar + #bytes + 8)
...
local String = {
    ...
    From = function(address)
        local pointToStr = gg.getValues({{address = libmain.cpp.FixValue(address), flags = libmain.cpp.MainType}})[1]
        local str = setmetatable(
            {
                address = libmain.cpp.FixValue(pointToStr.value), 
                Fields = {},
                pointToStr = libmain.cpp.FixValue(address)
            }, {__index = StringApi})
        local pointClassAddress = gg.getValues({{address = str.address, flags = libmain.cpp.MainType}})[1].value
        local stringInfo = libmain.cpp.FindClass({{Class = libmain.cpp.FixValue(pointClassAddress), FieldsDump = true}})[1]
