...
local ObjectApi = {
        ...
        for k, v in ipairs(gg.getValuesRange(RefToObjects)) do
            if v == 'A' then
                FilterObjects[#FilterObjects + 1] = {
                    address = libmain.cpp.FixValue(RefToObjects[k].value),
    ...
    FindHead = function(Address)
        local validAddress = libmain.cpp.GetValidAddress(Address)
        local mayBeHead = {}
        for i = 1, 1000 do
            mayBeHead[i] = {
        ...
        for i = 1, #mayBeHead do
            local mayBeClass = libmain.cpp.FixValue(mayBeHead[i].value)
            if libmain.cpp.ClassApi.IsClassInfo(mayBeClass) then
                return mayBeHead[i]
            end
        end
