local ClassInfoApi = {
    ...
    GetFieldWithName = function(self, name)
        ...
        if FieldsInfo then
            ...
            local _ClassInfo = gg.getValues({
                { -- Link as Fields
                    address = ClassAddress + libmain.cpp.ClassApi.FieldsLink,
                    flags = libmain.cpp.MainType
                },
                { -- Fields Count
                    address = ClassAddress + libmain.cpp.ClassApi.CountFields,
                    flags = gg.TYPE_WORD
                }
            })
            self.Fields = libmain.cpp.ClassApi:GetClassFields(libmain.cpp.FixValue(_ClassInfo[1].value), _ClassInfo[2].value, {
    ...
    GetMethodsWithName = function(self, name)
        ...
        if MethodsInfo then
            ...
            local _ClassInfo = gg.getValues({
                { -- Link as Methods
                    address = ClassAddress + libmain.cpp.ClassApi.MethodsLink,
                    flags = libmain.cpp.MainType
                },
                { -- Methods Count
                    address = ClassAddress + libmain.cpp.ClassApi.CountMethods,
                    flags = gg.TYPE_WORD
                }
            })
            self.Methods = libmain.cpp.ClassApi:GetClassMethods(libmain.cpp.FixValue(_ClassInfo[1].value), _ClassInfo[2].value,
                self.ClassName)
