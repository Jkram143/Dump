...
local ClassApi = {
    ...
    GetClassName = function(self, ClassAddress)
        return libmain.cpp.Utf8ToString(libmain.cpp.FixValue(gg.getValues({{
            address = libmain.cpp.FixValue(ClassAddress) + self.NameOffset,
    ...
    GetClassMethods = function(self, MethodsLink, Count, ClassName)
        ...
        for i = 1, #_MethodsInfo do
            local MethodInfo
            MethodInfo, _MethodsInfo[i] = libmain.cpp.MethodsApi:UnpackMethodInfo({
                MethodInfoAddress = libmain.cpp.FixValue(_MethodsInfo[i].value),
                ClassName = ClassName
            })
            table.move(MethodInfo, 1, #MethodInfo, #MethodsInfo + 1, MethodsInfo)
        end
        MethodsInfo = gg.getValues(MethodsInfo)
        libmain.cpp.MethodsApi:DecodeMethodsInfo(_MethodsInfo, MethodsInfo)
        return _MethodsInfo
    end,
    ...
    GetClassFields = function(self, FieldsLink, Count, ClassCharacteristic)
        ...
        for i = 1, #_FieldsInfo do
            local FieldInfo
            FieldInfo = libmain.cpp.FieldApi:UnpackFieldInfo(libmain.cpp.FixValue(_FieldsInfo[i].address))
            table.move(FieldInfo, 1, #FieldInfo, #FieldsInfo + 1, FieldsInfo)
        end
        FieldsInfo = gg.getValues(FieldsInfo)
        _FieldsInfo = libmain.cpp.FieldApi:DecodeFieldsInfo(FieldsInfo, ClassCharacteristic)
        return _FieldsInfo
    end,
    ...
    UnpackClassInfo = function(self, ClassInfo, Config)
        ...
        local ClassName = ClassInfo.ClassName or libmain.cpp.Utf8ToString(libmain.cpp.FixValue(_ClassInfo[1].value))
        local ClassCharacteristic = {
            ClassName = ClassName,
            IsEnum = ((_ClassInfo[9].value >> self.EnumRsh) & 1) == 1,
            TypeMetadataHandle = libmain.cpp.FixValue(_ClassInfo[10].value)
        }
        return setmetatable({
            ClassName = ClassName,
            ClassAddress = string.format('%X', libmain.cpp.FixValue(ClassInfo.ClassInfoAddress)),
            Methods = (_ClassInfo[2].value > 0 and Config.MethodsDump) and
                self:GetClassMethods(libmain.cpp.FixValue(_ClassInfo[4].value), _ClassInfo[2].value, ClassName) or nil,
            Fields = (_ClassInfo[3].value > 0 and Config.FieldsDump) and
                self:GetClassFields(libmain.cpp.FixValue(_ClassInfo[5].value), _ClassInfo[3].value, ClassCharacteristic) or
                nil,
            Parent = _ClassInfo[6].value ~= 0 and {
                ClassAddress = string.format('%X', libmain.cpp.FixValue(_ClassInfo[6].value)),
                ClassName = self:GetClassName(_ClassInfo[6].value)
            } or nil,
            ClassNameSpace = libmain.cpp.Utf8ToString(libmain.cpp.FixValue(_ClassInfo[7].value)),
            StaticFieldData = _ClassInfo[8].value ~= 0 and libmain.cpp.FixValue(_ClassInfo[8].value) or nil,
            ...
            __index = libmain.cpp.ClassInfoApi,
    ...
    IsClassInfo = function(Address)
        local imageAddress = libmain.cpp.FixValue(gg.getValues(
            {
                {
                    address = libmain.cpp.FixValue(Address),
                    flags = libmain.cpp.MainType
                }
            }
        )[1].value)
        local imageStr = libmain.cpp.Utf8ToString(libmain.cpp.FixValue(gg.getValues(
            {
                {
    ...
    FindClassWithName = function(self, ClassName, searchResult)
        local ClassNamePoint = libmain.cpp.GlobalMetadataApi.GetPointersToString(ClassName)
        local ResultTable = {}
            ...
            for classPointIndex, classPoint in ipairs(ClassNamePoint) do
                ...
                if (imageName) then
                    ResultTable[#ResultTable + 1] = {
                        ClassInfoAddress = libmain.cpp.FixValue(classAddress),
                        ClassName = ClassName,
                        ImageName = imageName
                    }
                end
    ...
    Find = function(self, class)
        local searchResult = libmain.cppMemory:GetInformationOfClass(class.Class)
        if not searchResult 
        ...
        if searchResult.isNew then
            ...
            libmain.cppMemory:SetInformationOfClass(class.Class, searchResult)
        else
